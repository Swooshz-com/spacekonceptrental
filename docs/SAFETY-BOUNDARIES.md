...
It may call only the Phase 2B-Y route gate adapter. It must not read request headers directly, read cookies directly, read env directly, or call lower-level auth/security boundaries directly. It is not approval to add other route handlers, pages, server actions, login/logout routes, protected admin pages, admin UI, product writes, Storage, deployment config, Supabase Cloud, browser Supabase, service-role runtime paths, n8n changes, Pinecone runtime code, SaaS chatbot app code, or `website/chat-config.js` access.