...
derives all static public pages from the Next.js app tree, treats
`/admin/login` as an anonymously reachable public HTML page, classifies the
explicit protected-admin/API/dynamic set without inventing parameters, and
fails closed on new unclassified admin pages or unsupported route syntax.
...
that hosted build provenance matched both, came from a clean tracked build,
and had a complete valid asset-inventory digest. Local `website/.next` content
is not deployed-build evidence. The hosted manifest is generated by
`npm run build` from the complete local asset tree, rejecting symlinks and
excluding non-JavaScript entries, and every listed hosted asset is
