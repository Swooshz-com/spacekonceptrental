...
const phase6pHeading = '## Phase 6P-A/B Maintenance Closure Audit Follow-Up Response Acknowledgement Review Outcome Follow-Up Planning Review Outcome Acknowledgement Review Readiness References';
...
const readinessOnlyBody = 'Phase 6P-A/B keeps the repo readiness-only for owner/admin review readiness of future theoretical follow-up planning review outcome acknowledgement materials after Phase 6O-A/B follow-up planning review outcome acknowledgement readiness.';
const trackerRequired = [
  ...
  'protected admin maintenance closure audit follow-up response acknowledgement review outcome follow-up planning review outcome acknowledgement review readiness helper coverage',
  'Phase 6O-A/B remains the prior follow-up planning review outcome acknowledgement readiness phase',
  'Phase 6N-A/B remains the earlier follow-up planning review outcome readiness phase',
...
for (const requiredPath of docsPaths) assert(fs.existsSync(path.join(repoRoot, requiredPath)), `Phase 6P doc missing ${requiredPath}`);
for (const trackerPath of trackerPaths) {
  const tracker = read(trackerPath);
  for (const required of trackerRequired) includes(tracker, required, `${trackerPath} Phase 6P tracker section`);
  const headingIndex = tracker.indexOf(phase6pHeading);
  const nextHeadingIndex = tracker.indexOf('\n## ', headingIndex + phase6pHeading.length);
  const section = tracker.slice(headingIndex, nextHeadingIndex === -1 ? tracker.length : nextHeadingIndex);
  noMatch(section, /Current phase: Phase 6[ONMLKJI]-A\/B|Phase 6O-A\/B keeps the repo readiness-only for owner\/admin acknowledgement readiness of future theoretical follow-up planning review outcome materials/i, `${trackerPath} Phase 6P section`);
}
...
]) includes(docs, required, 'Phase 6P docs');

noMatch(docs, /follow-up planning review outcome acknowledgement review was selected|follow-up planning review outcome acknowledgement review was recorded|acknowledgement review decision was selected|acknowledgement review decision was recorded|follow-up planning review outcome acknowledgement was selected|follow-up planning review outcome acknowledgement was recorded|acknowledgement decision was selected|acknowledgement decision was recorded|follow-up planning review outcome was selected|follow-up planning review outcome was recorded|follow-up planning review decision was selected|follow-up planning review decision was recorded|follow-up planning decision was selected|follow-up planning decision was recorded|follow-up action was selected|follow-up action was recorded|follow-up owner was assigned|remediation was assigned|recipient was contacted|customer contact was sent|support contact was sent|outbound contact was sent|admin contact was sent|closure decision was recorded|archive was created|retention policy was applied|production evidence was recorded|actual deployment|monitoring configured|analytics configured|cron configured|job configured|deployment approval granted/i, 'Phase 6P docs');
for (const pattern of [
  /Phase 6P-A\/B admin-only maintenance closure audit follow-up response acknowledgement review outcome follow-up planning review outcome acknowledgement review readiness/i,
  /MaintenanceClosureAuditFollowUpResponseAcknowledgementReviewOutcomeFollowUpPlanningReviewOutcomeAcknowledgementReviewReadinessHelper/i,
  /Audit response acknowledgement review outcome follow-up planning review outcome acknowledgement review ledger/i,
  /Audit response acknowledgement review outcome follow-up planning review outcome acknowledgement review checklist/i,
  /No-planning-review-outcome-acknowledgement-review\/no-acknowledgement-review-decision\/no-follow-up-action\/no-contact\/no-remediation firewall/i,
  /Safe follow-up planning review outcome acknowledgement review language/i,
]) assert(pattern.test(admin), `Phase 6P admin source missing ${pattern}`);
...
const phase6pFunctionStart = admin.indexOf('function MaintenanceClosureAuditFollowUpResponseAcknowledgementReviewOutcomeFollowUpPlanningReviewOutcomeAcknowledgementReviewReadinessHelper()');
...
assert(phase6pFunctionStart !== -1, 'Phase 6P admin helper missing');
assert(ownerPanelStart !== -1, 'Owner readiness helpers panel missing');
assert(phase6oFunctionStart < phase6pFunctionStart && phase6pFunctionStart < ownerPanelStart, 'Phase 6P helper must be placed after the Phase 6O helper and before the owner readiness panel');
assert(/<MaintenanceClosureAuditFollowUpResponseAcknowledgementReviewOutcomeFollowUpPlanningReviewOutcomeAcknowledgementReadinessHelper \/>[\s\S]*<MaintenanceClosureAuditFollowUpResponseAcknowledgementReviewOutcomeFollowUpPlanningReviewOutcomeAcknowledgementReviewReadinessHelper \/>/.test(admin), 'Phase 6P helper must render immediately after Phase 6O in the protected admin helper panel');
includes(adminPage, 'view={{ kind: "home" }}', 'admin page home view');
assert(pkg.scripts?.['validate:maintenance-closure-audit-follow-up-response-acknowledgement-review-outcome-follow-up-planning-review-outcome-acknowledgement-review-readiness'] === 'node scripts/validate-maintenance-closure-audit-follow-up-response-acknowledgement-review-outcome-follow-up-planning-review-outcome-acknowledgement-review-readiness.cjs', 'package.json must register Phase 6P validator');
includes(suite, "validate:maintenance-closure-audit-follow-up-response-acknowledgement-review-outcome-follow-up-planning-review-outcome-acknowledgement-review-readiness", 'release-candidate suite');
noMatch(suite, /docker.*(skip|bypass)|SKIP_DOCKER|BYPASS_DOCKER|supabase.*(skip|bypass)/i, 'release-candidate suite');

const phase6pAdminEnd = admin.indexOf('function OwnerReadinessHelpersPanel()', phase6pFunctionStart);
assert(phase6pAdminEnd !== -1, 'Phase 6P admin helper boundary missing');
const phase6pAdmin = admin.slice(phase6pFunctionStart, phase6pAdminEnd);
const phaseSources = docs + '\n' + phase6pAdmin;
noMatch(phaseSources, /\b(cart|checkout|payment|purchase)\b/i, 'Phase 6P materials');
noMatch(phaseSources, /\b(booking|reservation|fulfilment|stock-reservation)\b/i, 'Phase 6P materials');
noMatch(phaseSources, /\borders?\b/i, 'Phase 6P materials');
noMatch(phaseSources, /create(Server)?Client|fetch\(|axios|POST \/api|runtime handler|provider call|scheduler job|cron job|RAG retrieval|public route|public UI|webhook URL|NEXT_PUBLIC_|SUPABASE_SERVICE_ROLE_KEY/i, 'Phase 6P materials');
