...
const responsePlanningReadiness = 'docs/content/LOCAL-MAINTENANCE-CLOSURE-AUDIT-FOLLOW-UP-RESPONSE-PLANNING-READINESS.md';
const responseOptionLedger = 'docs/content/LOCAL-MAINTENANCE-CLOSURE-AUDIT-RESPONSE-OPTION-LEDGER-TEMPLATE.md';
for (const requiredPath of [responsePlanningReadiness, responseOptionLedger]) {
  assert(fs.existsSync(path.join(repoRoot, requiredPath)), `Phase 6F doc missing ${requiredPath}`);
}

const docs = `${read(responsePlanningReadiness)}\n${read(responseOptionLedger)}`;
for (const required of [
  'Phase 6F-A/B adds maintenance closure audit follow-up response planning readiness, an audit response option ledger template, safe response planning language, and a no-response/no-remediation firewall',
...
]) includes(docs, required, 'Phase 6F docs');

noMatch(docs, /actual deployment|audit finding was received|audit finding was recorded|audit follow-up record was created|audit finding was classified|audit severity was assigned|triage owner was assigned|triage decision was recorded|response option was selected|audit response was drafted|audit response was approved|audit response was sent|remediation was assigned|remediation task was created|external disclosure was made|audit recipient was contacted|archive was created|archive record was written|retention policy was applied|closure decision was recorded|closure approval was recorded|maintenance was marked complete|production evidence was collected|smoke check was run|provider check was executed|runtime check was executed|customer follow-up was sent|support response was sent|public notice was published|monitoring configured|analytics configured|cron configured|job configured|maintenance was completed|deployment approval granted/i, 'Phase 6F docs');

const admin = read('website/app/admin/protected-admin-shell.tsx');
for (const pattern of [
  /Phase 6F-A\/B admin-only maintenance closure audit follow-up response planning readiness/i,
  /MaintenanceClosureAuditFollowUpResponsePlanningReadinessHelper/i,
  /LOCAL-MAINTENANCE-CLOSURE-AUDIT-FOLLOW-UP-RESPONSE-PLANNING-READINESS\.md/i,
  /LOCAL-MAINTENANCE-CLOSURE-AUDIT-RESPONSE-OPTION-LEDGER-TEMPLATE\.md/i,
  /Audit response option ledger/i,
  /Audit response planning readiness checklist/i,
  /No-response\/no-remediation firewall/i,
  /Safe response planning language/i,
]) assert(pattern.test(admin), `Phase 6F admin source missing ${pattern}`);
assert(/<MaintenanceClosureAuditFollowUpTriageReadinessHelper \/>[\s\S]*<MaintenanceClosureAuditFollowUpResponsePlanningReadinessHelper \/>/.test(admin), 'Phase 6F helper must follow Phase 6E audit follow-up triage helper');
includes(read('website/app/admin/page.tsx'), 'view={{ kind: "home" }}', 'admin page');

const pkg = JSON.parse(read('package.json'));
assert(pkg.scripts?.['validate:maintenance-closure-audit-follow-up-response-planning-readiness'] === 'node scripts/validate-maintenance-closure-audit-follow-up-response-planning-readiness.cjs', 'package.json must register Phase 6F validator');
...
noMatch(publicSource, /maintenance closure audit follow-up|audit response option ledger|audit finding classification ledger|maintenance closure audit handoff|maintenance closure archive|closure archive retention ledger|maintenance closure decision|closure recommendation packet|support follow-up|storage provider|scheduler\/cron|provider setup|environment\/secrets|admin route|release-control|owner handoff|\/admin\//i, 'public source');
...
console.log('Phase 6F maintenance closure audit follow-up response planning readiness checks passed. No deployment was performed. No response option was selected. No audit response was drafted, approved, or sent. No remediation was assigned.');
