...
const addedTextWithoutThisValidator = getAddedDiffText(
  changedFiles.filter(
    (file) => file !== 'scripts/validate-protected-admin-enquiry-inbox-triage-foundation.cjs',
  ),
);
...
const addedRuntimeText = getAddedDiffText(
  changedFiles.filter(
    (file) =>
      ...
      !file.includes('.test.') &&
      file !== 'scripts/validate-protected-admin-enquiry-inbox-triage-foundation.cjs',
  ),
);
...
console.log(
  'Protected admin enquiry inbox triage foundation validation passed. Protected admin visibility, source metadata, CRM placeholders, docs, tests, and release-candidate wiring are present without provider calls, credentials, public customer access, or Docker skip/bypass.',
);
