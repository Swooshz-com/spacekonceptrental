...
## Current Product Direction Guardrails

This product is a furniture/event rental website.

Keep the visible MVP direction focused on:

```text
Public catalogue -> listing detail -> quote/enquiry submit -> protected admin triage
```

Do not add ecommerce, cart, checkout, order, payment, purchase, booking,
reservation, fulfilment, stock-reservation, customer account/dashboard, or
custom CRM wording or flows unless explicitly approved.

Do not mutate live external services without explicit current-turn approval
naming the target and allowed operation.
