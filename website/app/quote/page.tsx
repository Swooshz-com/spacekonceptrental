...
import { getDemoProducts, isDemoContentEnabled, productCategory, productSummary, stitchImages } from "../../components/PublicStitch";
...
function shouldShowDemoQuoteSelection({ category, event, product, requestedSlug, search }: { category?: string; event?: string; product: QuoteSelectionProduct | null; requestedSlug?: string; search?: string }) { return isDemoContentEnabled() && !product && !requestedSlug && !category && !event && !search; }
...
function SelectionPanel({ product, requestedSlug, category, event, search }: { product: QuoteSelectionProduct | null; requestedSlug?: string; category?: string; event?: string; search?: string }) {
  const demoItems = shouldShowDemoQuoteSelection({ category, event, product, requestedSlug, search }) ? getDemoProducts().slice(0, 2) : [];
  return <section className="stitch-quote-card stitch-quote-selection"><p className="stitch-eyebrow">Your Selection</p><h2>Your Selection</h2>{product ? <><p>{productSummary(product)}</p><dl className="stitch-facts"><div><dt>Selected Rental Items</dt><dd>{product.name}</dd></div><div><dt>Category</dt><dd>{productCategory(product)}</dd></div></dl><Link className="stitch-link-button stitch-link-button--quiet" href={`/catalogue/${product.slug}`}>View Details</Link></> : demoItems.length ? <><div className="stitch-selection-group"><h3>Selected Rental Items</h3>{demoItems.map((item, index) => <article className="stitch-selection-row" key={item.id}><img alt={`${item.name} thumbnail`} src={index === 0 ? stitchImages.chairImage.src : stitchImages.corporateImage.src} /><div><strong>{item.name}</strong><small>Qty: {index === 0 ? "4" : "1"}</small></div><span aria-hidden="true">×</span></article>)}</div><div className="stitch-selection-group"><h3>Selected Setups</h3><article className="stitch-selection-row"><span className="stitch-selection-row__icon" aria-hidden="true">?</span><div><strong>Minimalist Lounge Setup</strong><small>Qty: 1</small></div><span aria-hidden="true">×</span></article></div></> : <><p>{requestedSlug ? "The listing link may be old or unavailable. Keep this reference as editable request text if it still describes what you need." : "Share the requested pieces or setup direction you have in mind. The team can review your event context and follow up directly."}</p>{(category || event || search) ? <p>Discovery context is editable request intake only. Adjust the requested listings or items before sending.</p> : null}{(requestedSlug || category || event || search) ? <dl className="stitch-facts">{requestedSlug ? <div><dt>Selected listing reference</dt><dd>{requestedSlug}</dd></div> : null}{category ? <div><dt>Category</dt><dd>{category}</dd></div> : null}{event ? <div><dt>Event details</dt><dd>{event}</dd></div> : null}{search ? <div><dt>Search</dt><dd>{search}</dd></div> : null}</dl> : null}</>}</section>;
}
