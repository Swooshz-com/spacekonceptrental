...
export function StitchContactPage() {
  const supportRows = [
    ["Catalogue guidance", "Start with rental pieces, categories, or listing links you are considering."],
    ["Setup planning", "Share the room mood, guest flow, and event context you want the team to review."],
    ["Rental details", "Add dates, venue notes, quantities, access notes, and alternatives in the quote form."]
  ];

  return <><section className="stitch-contact-hero"><div className="stitch-container"><StitchPageIntro eyebrow="Contact" title="Get in Touch" intro="Share rental catalogue questions, setup context, or event notes through the enquiry path so the team can review the details and follow up." /></div></section><section className="stitch-section stitch-contact-section"><div className="stitch-container stitch-contact-grid"><aside className="stitch-contact-panel stitch-contact-copy"><h2>Contact Us</h2><p>Have a question about the rental catalogue or need a custom setup? Start with the rental brief path so the team can review your selected pieces, event details, and setup direction together.</p><div className="stitch-contact-support-card"><h2>Enquiry-led support</h2><p>Tell us about your event, rental needs, and preferred setup direction through the quote enquiry path.</p><div><strong>Manual review</strong><span>Our rental team reviews your selection and follows up with practical next steps.</span></div><Link href="/quote">Share event details</Link></div><dl><div><dt>Enquiry support</dt><dd>Use the quote form</dd></div><div><dt>Catalogue review</dt><dd>Share selected rental pieces</dd></div></dl></aside><aside className="stitch-contact-brief-panel" aria-label="Rental brief guidance"><p className="stitch-eyebrow">Rental enquiry path</p><h2>Start with a rental brief</h2><p>The quote form is the best place to share selected pieces, setup notes, date or venue context, and the details the team should review before preparing a tailored proposal.</p><div className="stitch-contact-brief-actions"><Link className="stitch-button stitch-button--primary" href="/quote">Request Quote</Link><Link className="stitch-button stitch-button--secondary" href="/catalogue">Browse Catalogue</Link><Link className="stitch-button stitch-button--secondary" href="/listings">Explore Setups</Link></div><div className="stitch-contact-brief-list">{supportRows.map(([title, text]) => <article key={title}><span aria-hidden="true" /><div><h3>{title}</h3><p>{text}</p></div></article>)}</div><p className="stitch-contact-brief-note">This request path supports enquiry intake only. Final rental details are reviewed by the team during follow-up.</p></aside></div></section></>;
}
