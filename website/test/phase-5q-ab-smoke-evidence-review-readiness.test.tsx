...
describe("Phase 5Q-A/B smoke evidence review readiness", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it("renders the protected Phase 5Q helper and full helper chain for authorised admin home state", () => {
    ...
    expect(screen.getByText(/public route result review placeholder/i)).toBeInTheDocument();
    expect(screen.getByText(/go\/no-go decision ledger placeholders/i)).toBeInTheDocument();
    expect(screen.getAllByText(/decision id: \[not assigned\]/i).length).toBeGreaterThan(0);
    expect(screen.getByText(/allowed future evidence review statuses/i)).toBeInTheDocument();
    expect(screen.getByText(/ready for future go\/no-go review/i)).toBeInTheDocument();
  ...
  it("does not render the protected Phase 5Q helper for blocked admin states", () => {
    ...
    for (const state of blockedStates) {
      ...
      expect(
        screen.queryByRole("heading", {
          name: /smoke evidence review readiness helper/i,
        }),
      ).not.toBeInTheDocument();
      expect(screen.queryByText(smokeEvidenceReviewReadinessDocPath)).not.toBeInTheDocument();
      expect(screen.queryByText(goNogoDecisionLedgerTemplateDocPath)).not.toBeInTheDocument();

      unmount();
    }
  });

  it("keeps protected admin source wired to Phase 5Q and Phase 5P docs", () => {
    ...
    expect(adminSource).toMatch(/Phase 5Q-A\/B admin-only smoke evidence review readiness/i);
  });
  ...
  it("keeps Phase 5Q docs template-only with no evidence and no deployment approval claim", () => {
    const docs = `${readRepoFile(smokeEvidenceReviewReadinessDocPath)}\n${readRepoFile(
  ...
  it("registers the Phase 5Q validator and keeps the release suite free of Docker bypass logic", () => {
    ...
    expect(
      packageJson.scripts?.["validate:smoke-evidence-review-readiness"],
    ).toBe("node scripts/validate-smoke-evidence-review-readiness.cjs");
