...
describe("Phase 4A-A/B local release-control gate", () => {
  ...
  it("renders the release-control workspace only for authorised admin state", () => {
    ...
    expect(screen.getByRole("heading", { name: /phase 4a-a\/b release-control gate/i })).toBeInTheDocument();
    expect(screen.getByText("Current phase")).toBeInTheDocument();
    expect(screen.getByText("Phase 4A-A/B")).toBeInTheDocument();
    expect(screen.getByText("Last merged capability PR")).toBeInTheDocument();
    expect(screen.getByText("#148")).toBeInTheDocument();
    expect(screen.getByText(phase3zMergeCommit)).toBeInTheDocument();
    ...
    for (const state of [
      ...
      expect(screen.queryByRole("heading", { name: /phase 4a-a\/b release-control gate/i })).not.toBeInTheDocument();
