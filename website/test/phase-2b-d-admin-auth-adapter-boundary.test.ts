...
describe("Phase 2B-D admin auth adapter boundary", () => {
  it("adds only the server-only adapter boundary and keeps docs honest", () => {
    expect(existsSync(resolve(repoRoot, adapterPath))).toBe(true);

    const adapter = readRepoFile(adapterPath);
    ...
    expect(adapter).toContain("AdminAuthAdapter");
    expect(adapter).toContain("AdminProfileAdapter");
    expect(adapter).toContain("AdminMembershipAdapter");
  ...
  it("keeps resolver and adapter modules side-effect free and local-only", () => {
    const combinedSource = [adapterPath, resolverPath].map(readRepoFile).join("\n");

    expect(combinedSource).not.toContain("@supabase/");
    expect(combinedSource).not.toContain("createServerSupabaseClient");
    ...
    expect(combinedSource).not.toMatch(/\bRequest\s*[<({]/);
    expect(combinedSource).not.toMatch(/\bResponse\s*[<({]/);
