...
type AdminQuoteRequestHubSpotImportCsvRouteEnv = {
  ADMIN_EXPECTED_ORIGIN?: string | null;
  ADMIN_EXPECTED_HOST?: string | null;
  ADMIN_TRUSTED_WORKSPACE_ID?: string | null;
};

export type AdminQuoteRequestHubSpotImportCsvRouteDependencies = {
  env?: AdminQuoteRequestHubSpotImportCsvRouteEnv;
  generatedAt?: () => string;
  now?: () => number;
...
function getRouteEnv(
  dependencies: AdminQuoteRequestHubSpotImportCsvRouteDependencies
) {
...
export async function handleAdminQuoteRequestHubSpotImportCsvRoute(
  request: NextRequest,
  dependencies: AdminQuoteRequestHubSpotImportCsvRouteDependencies = {}
  ...
  const csvResult = generateAdminQuoteRequestHubSpotImportCsv(result.packet);

  if (csvResult.status !== "generated") {
    return errorJson("quote_crm_handoff_csv_unavailable", 503);
  }
