...
import { generateAdminQuoteRequestHubSpotImportCsvPreflight } from "./admin-quote-request-hubspot-import-csv-preflight";
import {
  ...
  type AdminQuoteRequestHubSpotManualImportOutcomeReadResult
} from "./admin-quote-request-hubspot-manual-import-outcome-ledger";
import { generateAdminQuoteRequestHubSpotSyncDryRunContract } from "./admin-quote-request-hubspot-sync-dry-run-contract";

type AdminQuoteRequestHubSpotSyncDryRunContractRouteEnv = {
  ADMIN_EXPECTED_ORIGIN?: string | null;
  ADMIN_EXPECTED_HOST?: string | null;
  ADMIN_TRUSTED_WORKSPACE_ID?: string | null;
};

export type AdminQuoteRequestHubSpotSyncDryRunContractRouteDependencies = {
  env?: AdminQuoteRequestHubSpotSyncDryRunContractRouteEnv;
  generatedAt?: () => string;
  now?: () => number;
...
function getRouteEnv(
  dependencies: AdminQuoteRequestHubSpotSyncDryRunContractRouteDependencies
) {
...
function packetError(result: AdminQuoteRequestCrmHandoffPacketReadResult) {
  switch (result.status) {
    ...
    case "loaded":
      return {
        error: "quote_crm_handoff_hubspot_sync_dry_run_contract_unavailable",
        status: 503
      };
  }
}
...
function metadataError(
  result:
    | AdminQuoteRequestCrmHandoffPacketManifestReadResult
    | AdminQuoteRequestHubSpotManualImportOutcomeReadResult
) {
  switch (result.status) {
...
export async function handleAdminQuoteRequestHubSpotSyncDryRunContractRoute(
  request: NextRequest,
  dependencies: AdminQuoteRequestHubSpotSyncDryRunContractRouteDependencies = {}
  ...
  if (result.status !== "loaded") {
    const mapped = packetError(result);
  ...
  const manifestPersistence =
    ...
    adminQuoteRequestCrmHandoffPacketManifestPersistence;
  const manifestResult = await manifestPersistence.readRecentManifests({
    admin: binding.adminContext,
    limit: recentMetadataLimit
  });

  if (manifestResult.status !== "loaded") {
    const mapped = metadataError(manifestResult);

    return errorJson(mapped.error, mapped.status);
  }

  const outcomePersistence =
    dependencies.outcomePersistence ??
    adminQuoteRequestHubSpotManualImportOutcomeLedgerPersistence;
  const outcomeResult = await outcomePersistence.readRecentOutcomes({
    admin: binding.adminContext,
    limit: recentMetadataLimit
  });

  if (outcomeResult.status !== "loaded") {
    const mapped = metadataError(outcomeResult);

    return errorJson(mapped.error, mapped.status);
  }

  const preflight = generateAdminQuoteRequestHubSpotImportCsvPreflight(
    result.packet
  );
  ...
  return NextResponse.json(
    {
      ok: true,
      dryRunContract: generateAdminQuoteRequestHubSpotSyncDryRunContract({
        generatedAt,
        workspaceId: binding.adminContext.workspaceId,
        limit,
        packet: result.packet,
        manifests: manifestResult.manifests,
        manualImportOutcomes: outcomeResult.outcomes,
        preflight
      })
    },
    {
