...
import {
  ...
  type AdminQuoteRequestHubSpotSyncDryRunContractRouteDependencies
} from "./admin-quote-request-hubspot-sync-dry-run-contract-route";
...
function request(
  path = "/api/admin/quote-requests/crm-handoff-packet/hubspot-sync-dry-run-contract"
) {
...
function requestWithMethod(method: string) {
  return new Request(
    "https://admin.space.test/api/admin/quote-requests/crm-handoff-packet/hubspot-sync-dry-run-contract",
    {
      method,
      headers: {
        origin: env.ADMIN_EXPECTED_ORIGIN,
...
type TestDependencies =
  AdminQuoteRequestHubSpotSyncDryRunContractRouteDependencies & {
    resolveRouteGate: NonNullable<
      AdminQuoteRequestHubSpotSyncDryRunContractRouteDependencies["resolveRouteGate"]
    >;
...
describe("admin HubSpot sync dry-run contract route helper", () => {
  it("returns a no-store bounded dry-run contract after protected admin CSRF checks without mutations or provider calls", async () => {
    ...
    const response =
      await handleAdminQuoteRequestHubSpotSyncDryRunContractRoute(
  ...
  it.each([
  ...
  ])("returns safe JSON for invalid dry-run contract requests", async (
    input,
    expectedStatus,
    expectedError
  ) => {
    ...
    expect(dependencies.manifestPersistence.readRecentManifests)
      .not.toHaveBeenCalled();
    expect(dependencies.outcomePersistence.readRecentOutcomes)
      .not.toHaveBeenCalled();
  });
  ...
  it("maps local dry-run failures to generic errors without leaking SQL, provider, token, session, or header details", async () => {
    const dependencies = createDependencies(
